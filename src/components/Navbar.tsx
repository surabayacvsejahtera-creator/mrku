    ...
    >
      <div className="container flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-7">
          ...
          <div className="relative" ref={themeRef}>
            <motion.button
              ...
              className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
              aria-label="Ganti tema warna"
              title={`Tema: ${DAY_NAMES[dayIndex]}`}
              whileTap={{ scale: 0.95 }}
            >
              <Palette className="h-4 w-4" />
              <span className="text-xs font-semibold">{isOverridden ? `${DAY_NAMES[dayIndex]} (Manual)` : DAY_NAMES[dayIndex]}</span>
            ...
            <AnimatePresence>
              {themeOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 8, scale: 0.95 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 top-full mt-3 w-64 glass-white rounded-2xl shadow-blue-lg overflow-hidden border border-border/50 p-2"
                >
                  <div className="px-2 pb-2">
                    <div className="text-xs font-semibold text-foreground">Tema Hari Ini: {DAY_NAMES[dayIndex]}</div>
                    <div className="text-[11px] text-muted-foreground">{isOverridden ? "Mode manual aktif" : "Mode otomatis aktif mengikuti hari"}</div>
                  </div>
                  <button
        ...
        <div className="flex items-center gap-1 md:hidden">
          ...
          >
            <Palette className="h-4 w-4" />
            <span className="text-[11px] font-semibold">{DAY_NAMES[dayIndex]}</span>
            <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
          </button>
          <button
            onClick={toggleMute}
